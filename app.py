...
#     app.run(debug=True) #set port = 1234 (asal tidak dipakai aplikasi lain)
...
def preprocessing(filename):
    df = pd.read_csv('prepro/' + filename)
...
def predict(filename):
    ...
    (pfizer_total_pos, pfizer_total_neg) = naive_bayes('vaksin_moderna_df.csv')
